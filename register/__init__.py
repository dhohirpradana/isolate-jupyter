...
def handler(username):
    ...
    try:
        check_svc = check_service(f"jupyter-{replacement_string}")
        print(check_svc)
        if check_svc == '1':
            return jsonify({'error': f'Service {replacement_string} already exists!'}), 409
        ...
        try:
            ...
            modified_content = input_content.replace('SERVICE_NAME', replacement_string).replace(service_port, f'{unused_port_result}')
            ...
            try:
                if request.method == 'POST':
                    ...
                    return jsonify({"message": f"Register user {replacement_string} successfully!", "port": unused_port_result}), 200
                
                elif request.method == 'DELETE':
                    rmdir(f'/usersapujagad/{service_name}')
                    user_remove(service_name)
                    subprocess.run(["kubectl", "delete", "-f", output_filename], check=True, text=True)
                    return jsonify({"message": f"User {replacement_string} deleted successfully!"}), 200
